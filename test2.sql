...
/*
    릴레이션 : 관련된 데이터를 모아준 곳(회원,게시판,맛집,여행,쇼핑...)
          ...
          특징 1) 유일값을 가지고 있다 : 같은 데이터베이스 내에서 같은 이름 사용 불가능
                 2) 컬럼은 순서가 없다(컬럼+데이터값)
                 3) 데이터 순서가 없다(정렬)
                 4) 이차원 구조(컬럼,컬럼에 해당되는 데이터값)
                 5) 테이블(릴레이션) 구조(릴레이션 제어=> 단위 recode, ROW)
                       ----------------------------------
                       ...
                       ----------------------------------
                        72page
                         ...
                         74page
                             ...
                             = 속성명은 다른 명칭을 가지고 있다
                             = 속성은 순서가 없다
                             = 한개의 데이터베이스 안에 같은 데이블이 존재하면 안된다
                             = 튜플은 순서가 없다 => ORDER BY
                           76 키
                               1. 중복된 데이터가 없게 만든다
                                  --------------------------------- 이상현상 방지
